...
XWindowsScreen::registerHotKey (KeyID key, KeyModifierMask mask) {
    ...
    if (err) {
        ...
        for (HotKeyList::iterator j = hotKeys.begin (); j != hotKeys.end ();
             ++j) {
...
XWindowsScreen::handleSystemEvent (const Event& event, void*) {
    ...
    if (m_isPrimary) {
        if (xevent->type == KeyRelease) {
            ...
            XEvent xevent2;
            isRepeat = (XCheckIfEvent (m_display,
                                       &xevent2,
        ...
        default:
            ...
            if (m_xrandr) {
                if (xevent->type == m_xrandrEventBase + RRScreenChangeNotify ||
                    ...
                    // problem where moving
                    // off the server onto the client causes the pointer to warp
                    // to the
                    // center of the server (so you can't move the pointer off
                    // the server)
                    if (m_isPrimary) {
...
XWindowsScreen::detectXI2 () {
    ...
    return XQueryExtension (
        m_display, "XInputExtension", &xi_opcode, &event, &error);
}
