...
Server::Server (Config& config, PrimaryClient* primaryClient,
    ...
    m_events->adoptHandler (m_events->forIPrimaryScreen ().motionOnSecondary (),
                            ...
                            new TMethodEventJob<Server> (
                                this, &Server::handleMotionSecondaryEvent));
    m_events->adoptHandler (
        m_events->forIPrimaryScreen ().wheel (),
        m_primaryClient->getEventTarget (),
    ...
    m_events->adoptHandler (m_events->forServer ().switchInDirection (),
                            m_inputFilter,
                            new TMethodEventJob<Server> (
                                this, &Server::handleSwitchInDirectionEvent));
    m_events->adoptHandler (m_events->forServer ().keyboardBroadcast (),
                            m_inputFilter,
                            new TMethodEventJob<Server> (
                                this, &Server::handleKeyboardBroadcastEvent));
...
Server::setConfig (const Config& config) {
    ...
    if (!m_config->hasLockToScreenAction ()) {
        ...
        InputFilter::Rule rule (
            new InputFilter::KeystrokeCondition (m_events, key));
        rule.adoptAction (new InputFilter::LockCursorToScreenAction (m_events),
                          true);
...
Server::adoptClient (BaseClientProxy* client) {
    ...
    if (!m_config->isScreen (client->getName ())) {
        LOG ((CLOG_WARN "unrecognised client name \"%s\", check server config",
    ...
    if (!addClient (client)) {
        ...
        LOG ((CLOG_WARN "a client with name \"%s\" is already connected",
...
Server::disconnect () {
    ...
    if (m_clients.size () > 1 || !m_oldClients.empty ()) {
        ...
        m_events->addEvent (
            Event (m_events->forServer ().disconnected (), this));
    }
}
...
void
Server::switchScreen (BaseClientProxy* dst, SInt32 x, SInt32 y,
                      bool forScreensaver) {
    assert (dst != NULL);
    ...
    LOG ((CLOG_INFO "switch from \"%s\" to \"%s\" at %d,%d",
        ...
        if (m_active == m_primaryClient && m_enableClipboard) {
            for (ClipboardID id = 0; id < kClipboardEnd; ++id) {
                ...
                if (clipboard.m_clipboardOwner == getName (m_primaryClient)) {
                    onClipboardChanged (
                        m_primaryClient, id, clipboard.m_clipboardSeqNum);
                }
            }
        }
...
float
Server::mapToFraction (BaseClientProxy* client, EDirection dir, SInt32 x,
                       SInt32 y) const {
    SInt32 sx, sy, sw, sh;
...
void
Server::mapToPixel (BaseClientProxy* client, EDirection dir, float f, SInt32& x,
                    SInt32& y) const {
    SInt32 sx, sy, sw, sh;
    client->getShape (sx, sy, sw, sh);
    switch (dir) {
...
BaseClientProxy*
Server::getNeighbor (BaseClientProxy* src, EDirection dir, SInt32& x,
                     SInt32& y) const {
    ...
    LOG ((CLOG_DEBUG2 "find neighbor on %s of \"%s\"",
          Config::dirName (dir),
          srcName.c_str ()));
    ...
    for (;;) {
        ...
        if (dstName.empty ()) {
            LOG ((CLOG_DEBUG2 "no neighbor on %s of \"%s\"",
                  Config::dirName (dir),
                  srcName.c_str ()));
            return NULL;
        }
        ...
        if (index != m_clients.end ()) {
            LOG ((CLOG_DEBUG2 "\"%s\" is on %s of \"%s\" at %f",
                  dstName.c_str (),
                  Config::dirName (dir),
                  srcName.c_str (),
                  t));
        ...
        LOG ((CLOG_DEBUG2 "ignored \"%s\" on %s of \"%s\"",
              dstName.c_str (),
              Config::dirName (dir),
              srcName.c_str ()));
        srcName = dstName;
...
BaseClientProxy*
Server::mapToNeighbor (BaseClientProxy* src, EDirection srcSide, SInt32& x,
                       SInt32& y) const {
    // note -- must be locked on entry
    ...
    switch (srcSide) {
        case kLeft:
            ...
            while (dst != NULL) {
                ...
                LOG ((CLOG_DEBUG2 "skipping over screen %s",
                      getName (dst).c_str ()));
...
void
Server::avoidJumpZone (BaseClientProxy* dst, EDirection dir, SInt32& x,
                       SInt32& y) const {
...
bool
Server::isSwitchOkay (BaseClientProxy* newScreen, EDirection dir, SInt32 x,
                      SInt32 y, SInt32 xActive, SInt32 yActive) {
    LOG ((CLOG_DEBUG1 "try to leave \"%s\" on %s",
          getName (m_active).c_str (),
          Config::dirName (dir)));
    ...
    if (!allowSwitch && m_switchTwoTapDelay > 0.0) {
        if (isNewDirection || !isSwitchTwoTapStarted () ||
            !shouldSwitchTwoTap ()) {
...
UInt32
Server::getCorner (BaseClientProxy* client, SInt32 x, SInt32 y,
                   SInt32 size) const {
    assert (client != NULL);
...
Server::stopRelativeMoves () {
    if (m_relativeMoves && m_active != m_primaryClient) {
        ...
        m_xDelta2 = 0;
        m_yDelta2 = 0;
        LOG ((CLOG_DEBUG2 "synchronize move on %s by %d,%d",
              getName (m_active).c_str (),
              m_x,
              m_y));
...
Server::handleClipboardGrabbed (const Event& event, void* vclient) {
    ...
    if (grabber != m_primaryClient &&
        ...
        LOG ((CLOG_INFO "ignored screen \"%s\" grab of clipboard %d",
              getName (grabber).c_str (),
              info->m_id));
        return;
    }

    // mark screen as owning clipboard
    LOG ((CLOG_INFO "screen \"%s\" grabbed clipboard %d from \"%s\"",
          getName (grabber).c_str (),
          info->m_id,
...
Server::handleClientCloseTimeout (const Event&, void* vclient) {
    ...
    BaseClientProxy* client = static_cast<BaseClientProxy*> (vclient);
    LOG ((CLOG_NOTE "forced disconnection of client \"%s\"",
          getName (client).c_str ()));
    removeOldClient (client);
...
Server::handleSwitchInDirectionEvent (const Event& event, void*) {
    ...
    if (newScreen == NULL) {
        LOG ((CLOG_DEBUG1 "no neighbor %s",
              Config::dirName (info->m_direction)));
    } else {
...
Server::handleKeyboardBroadcastEvent (const Event& event, void*) {
    ...
    if (newState != m_keyboardBroadcasting ||
        ...
        LOG ((CLOG_DEBUG "keyboard broadcasting %s: %s",
...
Server::handleLockCursorToScreenEvent (const Event& event, void*) {
    ...
    if (newState != m_lockedToScreen) {
        ...
        LOG ((CLOG_NOTE "cursor %s current screen",
...
void
Server::onClipboardChanged (BaseClientProxy* sender, ClipboardID id,
                            UInt32 seqNum) {
    ClipboardInfo& clipboard = m_clipboards[id];
    ...
    if (seqNum < clipboard.m_clipboardSeqNum) {
        LOG ((CLOG_INFO
              "ignored screen \"%s\" update of clipboard %d (missequenced)",
    ...
    if (data == clipboard.m_clipboardData) {
        LOG ((CLOG_DEBUG
              "ignored screen \"%s\" update of clipboard %d (unchanged)",
              clipboard.m_clipboardOwner.c_str (),
              id));
        return;
    }
    ...
    // got new data
    LOG ((CLOG_INFO "screen \"%s\" updated clipboard %d",
          clipboard.m_clipboardOwner.c_str (),
          id));
...
Server::onScreensaver (bool activated) {
    LOG ((CLOG_DEBUG "onScreenSaver %s",
...
Server::onKeyDown (KeyID id, KeyModifierMask mask, KeyButton button,
                   const char* screens) {
    LOG ((CLOG_DEBUG1 "onKeyDown id=%d mask=0x%04x button=0x%04x",
          id,
          mask,
          button));
    assert (m_active != NULL);
...
Server::onKeyUp (KeyID id, KeyModifierMask mask, KeyButton button,
                 const char* screens) {
    LOG ((CLOG_DEBUG1 "onKeyUp id=%d mask=0x%04x button=0x%04x",
          id,
          mask,
          button));
    assert (m_active != NULL);
...
void
Server::onKeyRepeat (KeyID id, KeyModifierMask mask, SInt32 count,
                     KeyButton button) {
    LOG ((CLOG_DEBUG1 "onKeyRepeat id=%d mask=0x%04x count=%d button=0x%04x",
          id,
          mask,
          count,
          button));
    ...
    if (isSwitchOkay (newScreen, dir, x, y, xc, yc)) {
        if (m_args.m_enableDragDrop && m_screen->isDraggingStarted () &&
            ...
            if (m_sendDragInfoThread == NULL) {
                m_sendDragInfoThread = new Thread (new TMethodJob<Server> (
...
Server::sendDragInfo (BaseClientProxy* newScreen) {
    String infoString;
    UInt32 fileCount =
        DragInformation::setupDragInfo (m_dragFileList, infoString);
...
Server::onMouseMoveSecondary (SInt32 dx, SInt32 dy) {
    ...
    if (m_relativeMoves && isLockedToScreenServer ()) {
        LOG ((CLOG_DEBUG2 "relative move on %s by %d,%d",
              getName (m_active).c_str (),
              dx,
              dy));
    ...
    if (jump) {
        ...
        if (m_x < ax) {
            m_x = ax;
            LOG ((CLOG_DEBUG2 "clamp to left of \"%s\"",
                  getName (m_active).c_str ()));
        } else if (m_x > ax + aw - 1) {
            m_x = ax + aw - 1;
            LOG ((CLOG_DEBUG2 "clamp to right of \"%s\"",
                  getName (m_active).c_str ()));
        }
        if (m_y < ay) {
            m_y = ay;
            LOG ((CLOG_DEBUG2 "clamp to top of \"%s\"",
                  getName (m_active).c_str ()));
        } else if (m_y > ay + ah - 1) {
            m_y = ay + ah - 1;
            LOG ((CLOG_DEBUG2 "clamp to bottom of \"%s\"",
                  getName (m_active).c_str ()));
        }

        // warp cursor if it moved.
        if (m_x != xOld || m_y != yOld) {
            LOG ((CLOG_DEBUG2 "move on %s to %d,%d",
                  getName (m_active).c_str (),
                  m_x,
                  m_y));
            m_active->mouseMove (m_x, m_y);
...
Server::onFileRecieveCompleted () {
    if (isReceivedFileSizeValid ()) {
        m_writeToDropDirThread = new Thread (
            new TMethodJob<Server> (this, &Server::writeToDropDirThread));
    }
}
...
Server::addClient (BaseClientProxy* client) {
    ...
    m_events->adoptHandler (m_events->forIScreen ().shapeChanged (),
                            ...
                            new TMethodEventJob<Server> (
                                this, &Server::handleShapeChanged, client));
...
Server::forceLeaveClient (BaseClientProxy* client) {
    ...
    if (active == client) {
        ...
        // disconnected.
        LOG ((CLOG_INFO "jump from \"%s\" to \"%s\" at %d,%d",
...
Server::SwitchToScreenInfo::alloc (const String& screen) {
    SwitchToScreenInfo* info = (SwitchToScreenInfo*) malloc (
        sizeof (SwitchToScreenInfo) + screen.size ());
...
Server::sendFileToClient (const char* filename) {
    ...
    m_sendFileThread = new Thread (new TMethodJob<Server> (
        this,
        &Server::sendFileThread,
